...
const Section: React.FC<{ title: string; children: React.ReactNode; accentColor: string }> = ({ title, children, accentColor }) => (
    <div className="mb-4">
        <h3 className="text-lg font-bold border-b-2 pb-1 mb-2" style={{ color: accentColor, borderColor: accentColor }}>{title}</h3>
        {children}
    </div>
);

const MainSectionRenderer: React.FC<{ sectionKey: SectionKey; resume: ResumeData; accentColor: string }> = ({ sectionKey, resume, accentColor }) => {
    switch (sectionKey) {
        case 'experience':
                    ...
                    {resume.projects.map(proj => (
                        <div key={proj.id} className="mb-3">
                            <div className="flex justify-between items-baseline">
                                ...
                                <a href={proj.url} target="_blank" rel="noopener noreferrer" className="text-xs" style={{ color: accentColor }}>{proj.url}</a>
                            </div>
...
const SidebarSectionRenderer: React.FC<{ sectionKey: SectionKey; resume: ResumeData; accentColor: string }> = ({ sectionKey, resume, accentColor }) => {
     switch (sectionKey) {
        case 'skills':
                ...
                <Section title="Skills" accentColor={accentColor}>
                    <div className="flex flex-wrap gap-1 mt-1">
                        {resume.skills.map(skill => (
                            <span key={skill.id} className="bg-accent/20 text-primary rounded-md px-2 py-0.5 text-xs font-medium">
                                {skill.name}
                            </span>
                        ))}
                    </div>
...
export const ModernPreview: React.FC<{ resume: ResumeData }> = ({ resume }) => {
    ...
    return (
        <div className="flex h-full">
            {/* Sidebar */}
            <div className="w-1/3 p-4" style={{ backgroundColor: accentColor + '15' }}>
                <div className="text-center">
                    <h1 className="text-2xl font-bold" style={{color: accentColor}}>{personal.name}</h1>
                ...
                <div className="mt-6">
                    {sectionOrder.map(key => sidebarSections.includes(key) && <SidebarSectionRenderer key={key} sectionKey={key} resume={resume} accentColor={accentColor} />)}
                </div>
            </div>
            {/* Main Content */}
            <div className="w-2/3 p-6 overflow-y-auto">
                ...
                {sectionOrder.map(key => mainSections.includes(key) && <MainSectionRenderer key={key} sectionKey={key} resume={resume} accentColor={accentColor} />)}
            </div>
        </div>
    );
};